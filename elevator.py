...
def skippeoplemultiplecalls(end, elevatorposition,numberofpeopleontheelevator):
...
def pickupmultiplecalls(end, elevatorposition, numberofpeopleontheelevator):
    if end[0] > elevatorposition:
        ...
        elevatorposition = int(items)
def singlecall(end, elevatorposition, numberofpeopleontheelevator):
    end = int(end)
    elevatorposition = int(elevatorposition)
    numberofpeopleontheelevator = int(numberofpeopleontheelevator)
    if int(end) > elevatorposition:
        elevatorgoingup(elevatorposition, end, numberofpeopleontheelevator)
...
areaofelevator = int(lengthofelevator) * int(
widthofelevator)  # Calculates the area by multiplying the height by the width
...
areaofperson = int(statisticallyaveragedlengthofperson) * int(
statisticallyaveragedwidthofperson)  # Calculates the area of a person buy multiplying width by height
...
while(True):  # Final stages of the program, puts the elevator in a loop allowing it to go up and down and count the nunber of people in the elevator at every stop
    if type(end) == list and numberofpeopleontheelevator > maxcapacity:
        skippeoplemultiplecalls(end, elevatorposition, numberofpeopleontheelevator)
    elif type(end) == list:
        pickupmultiplecalls(end, elevatorposition,numberofpeopleontheelevator)
    else:
        singlecall(end, elevatorposition, numberofpeopleontheelevator)
